...
void eyeTracker::update(ofxCvGrayscaleImage & grayImgFromCam, float threshold, float minSize, float maxSize, float minSquareness) {
	...
	grayImgPreWarp.setFromPixels(grayImgFromCam.getPixels().getData(), grayImgFromCam.width, grayImgFromCam.height);		// TODO: there's maybe an unnecessary grayscale image (and copy) here...
	...
	if (bFoundOne && whoFound != -1) {
		...
		float *x = new float [MYN];
		float *y = new float[MYN];
		...
		if (bOk == true) {
			...
			for (int i = 0; i<resolution; i++) {
				ptsForRotation[i].rotate(theta * RAD_TO_DEG, ofVec2f(cx, cy));
			}
...
void eyeTracker::draw(float x, float y, float totalWidth, float totalHeight) {
	...
	if (currentEyePoint[0] != 0 && currentEyePoint[1] != 0 && currentEyePoint[2] != 0) {
		ofPushStyle();
		ofFill();
		ofSetColor(0, 0, 255);
